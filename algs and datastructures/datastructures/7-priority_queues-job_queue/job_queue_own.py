...
class JobQueue():
    def __init__(self):
        ...
        self._job_queue = [int(x) for x in sys.stdin.readline().split()]
        self._results = [(-1,-1) for _ in range(self.number_of_processes)]

    def process(self):
        self._data = [(idx, 0) for idx in range(self.number_of_threads)]
        for next_job_idx in range(self.number_of_processes):
            next_free_thread = self._data[0]
            self._results[next_job_idx] = next_free_thread
            next_job_duration = self._job_queue[next_job_idx]
            self._data[0] = (self._data[0][0], self._data[0][1] + next_job_duration)
            self.heapify(0)
    ...
    def heapify(self, i):
        ...
        min_idx = i
        if left_child_idx < len(self._data) and (self._data[left_child_idx][1] < self._data[min_idx][1] or
                                                     (self._data[left_child_idx][1] == self._data[min_idx][1] and self._data[left_child_idx][0] < self._data[min_idx][0])):
            min_idx = left_child_idx
        if right_child_idx < len(self._data) and (self._data[right_child_idx][1] < self._data[min_idx][1] or
                                                     (self._data[right_child_idx][1] == self._data[min_idx][1] and self._data[right_child_idx][0] < self._data[min_idx][0])):
    ...
    def print_results(self):
        for thread, start_time in self._results:
